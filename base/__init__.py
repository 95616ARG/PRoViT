...
class Dataset(torch.utils.data.Dataset):
    ...
    def sample_incorrect_label(self, network, label, num=1, shuffle=False):
        device, dtype = network.device, network.dtype
        remaining = num
        out = []
        out_labels = []
        from tqdm.auto import tqdm
        for images, labels in tqdm(self.dataloader(batch_size=1000, shuffle=shuffle)):
            if label is not None:
                mask = (labels.flatten() == label).cpu().numpy()
                if mask.sum() == 0:
                    continue
                images = images[mask]
                labels = labels[mask]
            ...
            mask = network(images).argmax(-1).reshape(-1).cpu() != labels.reshape(-1).cpu()
            if mask.any():
    ...
    def sample_correct_label(self, network, label, num=1, shuffle=False):
        device, dtype = network.device, network.dtype
        remaining = num
        out = []
        out_labels = []
        for images, labels in self.dataloader(batch_size=100, shuffle=shuffle):
            if label is not None:
                mask = (labels.flatten() == label).cpu().numpy()
                if mask.sum() == 0:
                    continue
    ...
    def sample_correct_labels(self, network, labels, num=1, keepdim=True, shuffle=False):
        out = []
        out_labels = []
        for label in labels:
            label_out, label_labels = self.sample_correct_label(network, label, num=num, shuffle=shuffle)
    ...
    def sample_incorrect_labels(self, network, labels, num=1, keepdim=True, shuffle=False):
        out = []
        out_labels = []
        for label in labels:
            label_out, label_labels = self.sample_incorrect_label(network, label, num=num, shuffle=shuffle)
            out.append(label_out)
            out_labels.append(label_labels)
        if keepdim:
            return torch.stack(out, 0), torch.stack(out_labels, 0)
    ...
    def classified(self, network, num, seed, device=None, dtype=None):
    ...
    def accuracy(self, network, topk=1, largest=True,
        device = None, dtype  = None, batch_size=100, num_workers=8,
        prefetch_factor=2, pin_memory=True, tqdm=True, **kwargs):
        device = device or self.device or next(iter(network.parameters())).device
        dtype  = dtype  or self.dtype  or next(iter(network.parameters())).dtype
        if device != next(iter(network.parameters())).device \
        or dtype  != next(iter(network.parameters())).dtype:
            network = copy.deepcopy(network).to(device=device).to(dtype=dtype)
