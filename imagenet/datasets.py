...
import PRoViT.base
from PRoViT.base import get_workspace_root
...
class Dataset(PRoViT.base.Dataset):
...
class ImageNet(Dataset):
    def __init__(self, root=(get_workspace_root() / 'data' / 'ILSVRC2012').as_posix(), split='val',
        transform=imagenet_transform,
        **kwargs):
        super().__init__(**kwargs)
        self.dataset = torchvision.datasets.ImageNet(root, split=split, transform=transform)

class ImageNet_C(Dataset):
    def __init__(self,
        root=(get_workspace_root() / 'data' / 'imagenet-c').as_posix(),
...
class ImageNet_A(Dataset):
    def __init__(self, root=(get_workspace_root() / 'data' / 'imagenet-a').as_posix(),
            transform=imagenet_transform,
            **kwargs):
            super().__init__(**kwargs)
    ...
    def __getitem__(self, idx) -> torch.Tensor:
        ...
        print("mapping NAE labels...")
        print(lbl)
        lbl = torch.tensor([indices_in_1k[i] for i in lbl.flatten()])
        print(lbl)
        return img, lbl
